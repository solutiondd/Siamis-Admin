    ...
    <div class="modal-box w-11/12 max-w-4xl">
      ...
      <div class="flex gap-2">
        <button class="btn btn-warning" @click="previewExcel" :disabled="!excelFile || isPreviewing">
          ...
          {{ $t('StudentImportExcal.previewBtn') }}
        </button>
      </div>
      ...
      <div v-if="previewData.length" class="mt-6">
          ...
          <table class="table table-zebra w-full table-sm">
            <thead>
              <tr class="bg-base-300">
                <th>{{ $t('StudentImportExcal.colCode') }}</th>
                <th>{{ $t('StudentImportExcal.colPrefix') }}</th>
                <th>{{ $t('StudentImportExcal.colFirstName') }}</th>
                <th>{{ $t('StudentImportExcal.colLastName') }}</th>
                <th>{{ $t('StudentImportExcal.colRfid') }}</th>
                <th>{{ $t('StudentImportExcal.colGrade') }}</th>
                <th>{{ $t('StudentImportExcal.colRoom') }}</th>
                <th>{{ $t('StudentImportExcal.colPhone') }}</th>
                <th>{{ $t('StudentImportExcal.colImage') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(student, idx) in pagedPreviewData" :key="idx">
                ...
                <td>
                  <span :class="student.imageMatched ? 'text-success font-semibold' : 'text-error font-semibold'">
                    {{ student.imageName || '-' }}
                  </span>
                </td>
              </tr>
        ...
        <button class="btn btn-success mt-4" @click="handleImport" :disabled="isImporting">
          ...
          {{ $t('StudentImportExcal.saveImportBtn') }}
        </button>
      </div>
